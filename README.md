...
- commenting intent and keeping documentation current;
- TSDoc/JSDoc for exported data-layer functions and documented `.astro` component `Props` contracts; and
- TypeScript formatting, type boundaries, and linting expectations.

Start with [`copilot-instructions.md`](.github/copilot-instructions.md), then follow the technology-specific instructions for the files you are changing.
...
The shared **Issue Triage Board** canvas (`.github/extensions/kanban-triage/`) loads open GitHub issues, highlights the three most urgent items, and lets team members add an issue directly to the current Copilot session context.
